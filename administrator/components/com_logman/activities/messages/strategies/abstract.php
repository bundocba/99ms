...
abstract class ComLogmanActivityMessageStrategyAbstract extends KObject implements ComLogmanActivityMessageStrategyInterface, KServiceInstantiatable
{
    ...
    final public function getParameters(KConfig $config)
    {
        ...
        if (preg_match_all('/%(.+?)%/', $text, $matches) !== false) {
            ...
            foreach ($matches[1] as $replacement) {
                ...
                $method = '_set' . ucfirst($replacement);

                if (method_exists($this, $method)) {

                    $parameter = $this->_getParameter(array('name' => $replacement));

                    call_user_func(array($this, $method),
                        new KConfig(array('parameter' => $parameter, 'activity' => $config->activity)));
    ...
    protected function _resourceExists($config = array())
    {
        ...
        $query = $db->getQuery()->from($config->table)->where($config->identity_column, '=', $activity->row)
            ->count();
    ...
    {
        return $this->getService($this->_parameter, $config);
    }

    protected function _setParameter(KConfig $config)
    {
        $config->append(array('translate' => false, 'attribs' => array()));

        $parameter = $config->parameter;

        if (!$parameter instanceof ComLogmanActivityMessageTranslatorParameterInterface) {
            throw new KException('Parameter object must be of ComLogmanActivityMessageTranslatorParameterInterface type');
        }

        if ($config->link) {
            $variables = array('url', 'attribs', 'route', 'absolute');
            $link      = array();
            foreach ($variables as $variable) {
                if (isset($config->link->{$variable})) {
                    $link[$variable] = KConfig::unbox($config->link->{$variable});
                }
            }
            $parameter->setLink($link);
        }

        $parameter->setText($config->text);
        $parameter->setTranslate($config->translate);
        $parameter->setAttribs(KConfig::unbox($config->attribs));
        $parameter->setMaxLength($config->max_length);
    }
}