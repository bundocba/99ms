...
class ComLogmanActivityMessageStrategyFiles extends ComLogmanActivityMessageStrategyDefault
{
        ...
        switch ($activity->name) {
            case 'file':
                if ($activity->package == 'docman') {
                    ...
                    $url = 'administrator/index.php?option=com_' . $activity->package . '&view=file&format=raw&routed=0&name=' . rawurlencode($path) . '&container=' . $metadata->container->slug;
                } else {
    ...
    protected function _setFolder(KConfig $config)
    {
        $activity = $config->activity;
        $metadata = $activity->metadata;
    ...
    protected function _setAction(KConfig $config)
    {
        ...
        return parent::_setAction($config);
    }
