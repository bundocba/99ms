...
class com_logmanInstallerScript
{
	...
	public function preflight($type, $installer)
	{
		...
		if (!class_exists('Koowa') || !class_exists('ComExtmanDatabaseRowExtension'))
		{
			...
			else $errors[] = JText::_('This component requires EXTman to be installed on your site. Please download this component from <a href=http://joomlatools.eu target=_blank>joomlatools.eu</a> and install it');
        ...
        if ($return === true) {
            if (version_compare($this->getExtmanVersion(), '1.0.0RC4', '<')) {
        ...
        if ($return == true && $type == 'update') {
            if ($current = $this->getCurrentVersion()) {
                // TODO The helper will be used for checks when EXTman 1.0.0RC5 is released.
                //$helper = new ComExtmanHelperInstaller();
                //if ($failed = $helper->checkDatabasePrivileges(array('ALTER'))) {
                if ($failed = $this->checkDatabasePrivileges(array('ALTER'))) {
     ...
     *
     * TODO Remove this method when EXTman 1.0.0RC5 is released (which will include this in its installer helper).
     *      Also make sure to make the right call within this object ($helper->checkDatabasePrivileges).
     *
