...
// 				vmdebug('$product->customfields',$product->customfields);
				...
				// MARTIN
				require_once JPATH_ROOT.DS.'includes'.DS.'fwsite.php';
				$product->fwquantity =  fwSite::getfwCustomByID($this->_id);
				// MARTIN
...
function getOrderByList($virtuemart_category_id=false) {
	...
	unset ($getArray['globalCurrencyConverter'],$getArray['search'],$getArray['keyword'],$getArray['order'],$getArray['orderby']);
	...
	if(VmConfig::get('show_manufacturers')){
		...
		// if ($mf_virtuemart_product_ids) {
			...
			if (count($manufacturers)>0) {
				...
				if ($virtuemart_manufacturer_id > 0) $manufacturerLink .='<div><a title="" href="'.JRoute::_($fieldLink.'&virtuemart_manufacturer_id='.$mf->virtuemart_manufacturer_id ) .'">'.JText::_('COM_VIRTUEMART_SEARCH_SELECT_ALL_MANUFACTURER').'</a></div>';
