...
// 		vmdebug('getOrderIdByOrderPass '.$orderId);
	...
	public function getOrder($virtuemart_order_id){
		...
		// Get the order items
/* Martin */
$q = 'SELECT colorid,sizename,virtuemart_order_item_id, product_quantity, order_item_name,
...
// 			vmdebug('updateSingleItem '.$virtuemart_order_item_id.' old: '.$oldOrderStatus.' new: '.$orderdata->order_status);
			...
			// Martin
			//var_dump($orderdata->order_status) ;
			
			 if($orderdata->order_status=='X'||$orderdata->order_status=='R'){

				if($table->virtuemart_product_id > 0 && $table->colorid > 0)
				{
					$db = JFactory::getDBO();
					$query="UPDATE `#__virtuemart_quantity` SET `color_ordered`=(`color_ordered` - ".$table->product_quantity.") WHERE `productid`=".$table->virtuemart_product_id." AND `colorid`=".$table->colorid." AND `sizename`='".$table->sizename."'";
					$db->setQuery($query);
					$db->query();
					$this->updateStock($table->virtuemart_product_id);
				}
			} 
			// Martin
...
// 			$_userInfoData['virtuemart_order_userinfo_id'] = null; // Reset key to make sure it doesn't get overwritten by ST
	...
	function handleStockAfterStatusChangedPerProduct($newState, $oldState,$product, $quantity) {
		...
		// $stockOut = array('S');
		if (isset($StatutWhiteList[$newState]['order_stock_handle']) && $StatutWhiteList[$newState]['order_stock_handle'] == 'O') $isOut = 1;
		else $isOut = 0;
		if (isset($StatutWhiteList[$oldState]['order_stock_handle']) && $StatutWhiteList[$oldState]['order_stock_handle'] == 'O') $wasOut = 1;
		else $wasOut = 0;
		...
		// $Reserved =  array('P','C');
		if (isset($StatutWhiteList[$newState]['order_stock_handle']) && $StatutWhiteList[$newState]['order_stock_handle'] == 'R') $isReserved = 1;
		else $isReserved = 0;
		if (isset($StatutWhiteList[$oldState]['order_stock_handle']) && $StatutWhiteList[$oldState]['order_stock_handle'] == 'R') $wasReserved = 1;
		else $wasReserved = 0;
...
// 			$_orderItems->virtuemart_userinfo_id = 'TODO'; //$_cart['BT']['virtuemart_userinfo_id']; // TODO; Add it in the cart... but where is this used? Obsolete?
			...
			/* Martin */
			$_orderItems->colorid = $_prod->colorid;
			$_orderItems->sizename = $_prod->sizename;
			/* Martin */
			...
			// Martin
			if($_prod->virtuemart_product_id > 0 && $_prod->colorid > 0 && !empty($_prod->sizename))
			{
				$db = JFactory::getDBO();
				$query="UPDATE `#__virtuemart_quantity` SET `color_ordered`=(`color_ordered` + ".$_orderItems->product_quantity.") WHERE `productid`=".$_prod->virtuemart_product_id." AND `colorid`=".$_prod->colorid." AND `sizename`='".$_prod->sizename."'";
				$db->setQuery($query);
				$db->query();
				$this->updateStock($_prod->virtuemart_product_id);
			}
			// Martin
...
// 			vmdebug('_createOrderLines',$_prod);
	...
	function updateStock($pdid){
		$pdid = (int)$pdid;
		$db = JFactory::getDBO();
		$query = "SELECT SUM(quantity) as product_in_stock, SUM(color_ordered) as product_ordered FROM #__virtuemart_quantity WHERE productid = $pdid";
		$db->setQuery($query);
		$result = $db->loadAssoc();
		
		$query="UPDATE `#__virtuemart_products` SET `product_ordered`=".$result['product_ordered'].", product_in_stock = ".$result['product_in_stock']." WHERE `virtuemart_product_id`=".$pdid;
		$db->setQuery($query);
