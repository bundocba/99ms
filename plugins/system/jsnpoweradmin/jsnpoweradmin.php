...
//include defines
if(file_exists(JPATH_ROOT . DS . 'administrator' . DS . 'components' . DS . 'com_poweradmin' . DS . 'poweradmin.defines.php')){
	require_once JPATH_ROOT . DS . 'administrator' . DS . 'components' . DS . 'com_poweradmin' . DS . 'poweradmin.defines.php' ;
}else if(file_exists(JPATH_ROOT . DS . 'administrator' . DS . 'components' . DS . 'com_poweradmin' . DS . 'defines.poweradmin.php')){
...
class plgSystemJsnpoweradmin extends JPlugin
{
	...
	**/
	...
	public function onAfterRoute(){
		...
		$uri	= JUri::root(true);
		if($option == 'com_modules' && $view == 'module'){
	...
	private function canEnablePosChooser()
	{
		...
		if($notSupportedTemplateAuthors == null)
			return false;
...
// 		}
	...
	private function _getJSConfiguration ()
	{
		...
		$conf = array(
			...
			'logoFile'			=> $logoFile,
			'logoLink'			=> $this->_params->get('logo_link', 'index.php?option=com_poweradmin&view=configuration'),
