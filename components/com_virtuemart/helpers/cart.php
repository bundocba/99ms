...
// 			$product -> product_price_edate = $tmpProduct -> product_price_edate;
			...
			/* Martin */
			$product -> colorid = (int) $post['color'];
			$product -> sizename = addslashes($post['size']);
			...
			if ($product) {
				...
				if (!empty($product ->colorid)) {
					$productKey = $product->virtuemart_product_id.'::'.$product ->colorid;
				}
				if (!empty($product->sizename)) {
					$productKey .= ":".$product->sizename;
				}
				/* End Martin */
...
// 		vmdebug('Calling getCartPrices',$this->pricesUnformatted);
	...
	*/
	...
	private function checkForQuantities($product, &$quantity=0,&$errorMsg ='') {
		...
		// Martin
		$post = JRequest::get('default');
		//check quantity color
		$colorid = (int) $post['color'];

		$sizename = '';
		if(!empty($post['size'])){
			$sizename = addslashes($post['size']);
		}
		
		if(JRequest::getVar('task')=='update')
		{
			$colorid=JRequest::getInt('color');
			$sizename = addslashes($_REQUEST['size']);
		}

		if($colorid && $sizename){
		$db = JFactory::getDBO();
		$query="Select * From #__virtuemart_quantity where colorid=".$colorid." AND sizename='".$sizename."'  AND productid=".$product->virtuemart_product_id ;
		$db->setQuery($query);
		$rows_state=$db->loadObjectList();
			if(count($rows_state))
			{
				if($quantity > ($rows_state[0]->quantity-$rows_state[0]->color_ordered)and $stockhandle='disableadd')
				{
					
					if(($rows_state[0]->quantity-$rows_state[0]->color_ordered)>0){
					$quantity = $rows_state[0]->quantity - $rows_state[0]->color_ordered;
					$quan=$quantity;
					$errorMsg = JText::sprintf('COM_VIRTUEMART_CART_PRODUCT_OUT_OF_QUANTITY',$quan,$quantity);
					$this->setError($errorMsg);
					vmInfo($errorMsg,$product->product_name);
...
// 			$this->data->products[$i]['prices'] = $this->prices[$priceKey]['subtotal_with_tax'];
		...
		$this->data->cart_empty_text  = JText::_('TM_VIRTUEMART_CART_EMPTY');
		$this->data->cart_recent_text  = JText::_('TM_VIRTUEMART_CART_ADD_RECENTLY');
		return $this->data ;
	}
}
